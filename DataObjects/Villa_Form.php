...
function codeAddress() {
        ...
        if (status == google.maps.GeocoderStatus.OK) {
         ...
         map.setOptions({draggableCursor: \"hand\"});
          marker = new google.maps.Marker({
            ...
            infowindow.setContent('".$this->form_help[4]."');
  ...
  function setRes(results) {
      var full_adr='';
        getEn(results[0].formatted_address);
      ...
      //alert(full_adr);
        document.getElementById(\"full_adr\").value = full_adr+';'+results[0].geometry.location;
        document.getElementById(\"address\").value = results[0].formatted_address;
        document.getElementById(\"next\").disabled = false;
  }

  function getEn(address) {
            geocoder.geocode( {   'address': address,
                            'language': 'en'
                         }, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
              ...
              alert(\"Geocode En was not successful: \" + status);
            }
       });
  }
...
";
        ...
        //$form->addElement('header', null, 'Расположение');
        $form->addElement('static', 'help', $this->form_help[1]);
        //$form->addElement('static', 'map', '<div align=center id="map_canvas" style="width:400px; height:400px; border:1px solid #ccc;"></div>');
        ...
        //$form->addElement('button', '', 'Уточнить положение', 'onclick="choosePosition();"');
    ...
    function findLocationJson($vals)
    {
        ...
        print_r($adr_arr);
        //utf8_decode($adr_arr->long_name[4]);
        ...
        */
        ...
        $vals['locations_all'] = ':';
        $parent_id = '';
        foreach (array_reverse($adr_arr->long_name_en, 1) as $k => $v) {
                if (preg_match("/political/", $adr_arr->types[$k]) && $v) {
                    /*
                    if ($v == 'St Petersburg' && $adr_arr->types[$k] == 'locality,political') {
                        continue;
                    }
                    if ($v == 'Moscow' && $adr_arr->types[$k] == 'locality,political') {
                        continue;
                    }
                    */
                    if ($v == 'Russian Federation') {
                        $v = 'Russia';
                    }
                    ...
                    if ($v == 'Israel' && $adr_arr->types[$k] == 'country,political') {
                        continue;
                            ...
                            $countries_obj = DB_DataObject::factory('countries');
                            //$countries_obj->whereAdd('rus_name="'.trim($adr_arr[4][$k]).'"');
                            //$countries_obj->name = trim(utf8_decode($v));
                            $countries_obj->whereAdd("name='".trim(utf8_decode($v))."' OR rus_name='".utf8_decode($adr_arr->long_name[$k])."'");
                            if ($parent_id) {
                            ...
                            if (!$countries_obj->N) {
                                if ($parent_id) {
                                    $countries_obj->name = trim(utf8_decode($v));
                                    $countries_obj->rus_name = utf8_decode($adr_arr->long_name[$k]);
    ...
    function findLocation($vals)
    {
        ...
        //$long_name_en = $this->englishName($adr_arr->location[0].','.$adr_arr->location[1], 'en');
        //$long_name_ru = $this->englishName($adr_arr->location[0].','.$adr_arr->location[1], 'ru');

        /*
        echo 'findLocation<pre>';
        echo $vals['full_adr'];
        print_r($vals);
        print_r($adr_arr);
        print_r($long_name_en);
        print_r($long_name_ru);
        utf8_decode($adr_arr->long_name[4]);
        DB_DataObject::DebugLevel(1);
        exit;
        */
